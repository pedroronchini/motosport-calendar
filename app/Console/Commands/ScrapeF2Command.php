...
use App\Services\Scrapers\F2Scraper;
use Illuminate\Console\Command;

class ScrapeF2Command extends Command
{
    /**
     * @var string
     */
    protected $signature = 'scrape:f2
     ...
     * @var string
     */
    protected $description = 'Popula o banco com o calendário de F2 raspando o site oficial (fiaformula2.com)';

    public function handle(F2Scraper $scraper): int
    {
        $year = (int) ($this->argument('year') ?: now()->year);
        $markCurrent = $this->option('current') || $year >= now()->year;
        $fetchResults = ! $this->option('no-results');

        $this->info("Buscando o calendário de F2 de {$year}...");
        $this->newLine();
